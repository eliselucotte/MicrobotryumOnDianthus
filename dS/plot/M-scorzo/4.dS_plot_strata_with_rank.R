...
MC12=ggplot(data=d2[which(d2$chrom_lag %in% PR_chrom),])+
  ...
  xlab('')+
  ylab('')+
  geom_vline(aes(xintercept= 69), col='red')

MC12_rankA1=ggplot(data=d2[which(d2$chrom_lag %in% PR_chrom),])+
  geom_point(aes(x=rank_lag,y=rankA1/coeff,color=chrom1))+
  #scale_x_continuous(breaks=seq(0,max(d2[which(d2$chrom_lag=='Mv-lag-1253-A1_MC12'),]$rank_lag), by = 0.1))+
  theme_bw()+
  theme(text = element_text(size = size_text))+
  #scale_colour_identity()+
...
MC12_rankA2=ggplot(data=d2[which(d2$chrom_lag %in% PR_chrom),])+
  geom_point(aes(x=rank_lag,y=rankA2/coeff,color=chrom2))+
  #scale_x_continuous(breaks=seq(0,max(d2[which(d2$chrom_lag=='Mv-lag-1253-A1_MC12'),]$rank_lag), by = 0.1))+
  theme_bw()+
  theme(text = element_text(size = size_text))+
  #scale_colour_identity()+
...
MC03A=ggplot(data=d2[which(d2$chrom_lag=='Mv-lag-1253-A1_MC03A'),], aes(x=rank_lag,y=dS))+
  ...
  theme(text = element_text(size = size_text))+
  ylim(0,0.3)+
  geom_vline(aes(xintercept= 40), col='red')+
  xlab('')+
  ggtitle('HD')
...
ggsave("dS_plot_M-scorzo-nooutlier_strata_with_ranklag.png", height=10, width=15)
ggsave("dS_plot_M-scorzo-nooutlier_strata_with_ranklag.pdf", device='pdf', height=10, width=15)
