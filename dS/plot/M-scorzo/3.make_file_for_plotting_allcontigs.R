...
new_A1=NULL
for(chrom in unique(A1$contig1))
{
...
new_A2=NULL
for(chrom in unique(A2$contig2))
{
...
write.table(d2, file='database_for_dSplot_M-scorzo_allcontig.txt', quote = F, row.names = F, sep='\t')

d2=read.table('database_for_dSplot_M-scorzo_allcontig.txt', header=T, as.is=T)
...
write.table(d2, file='database_for_dSplot_M-scorzo_withrank_allcontig.txt', quote = F, row.names = F, sep='\t')
...
write.table(d2, file='database_for_dSplot_M-scorzo_withrank_new_color_allcontig.txt', quote = F, row.names = F, sep='\t')
...
write.table(d3, file='database_for_strata_M-scorzo_allcontig.txt', quote = F, row.names = F, sep='\t')
