...
d_PR_genes=read.table('5.Rideogram/PR_genes.txt', header=T, as.is=T, sep='\t')
d=rbind(d,d_PR_genes[2,])

A1=d[which(d$chrom1 %in% chroms_keep_species2),]
A1=A1[which(A1$chrom_lag %in% chroms_keep_species1),]

contig1=read.table(paste('5.Rideogram/',specie1,'_contig.bed',sep=''), header=T, as.is=T, sep='\t')
contig1=contig1[which(contig1$Chr %in% unique(A1$chrom_lag)),]

contig2=read.table(paste('5.Rideogram/',specie2,'_contig.bed',sep=''), header=T, as.is=T, sep='\t')
...
t_plot=t_plot[order(t_plot$fill),]
ideogram(karyotype = contigs, synteny = t_plot, output=paste('5.Rideogram/',specie1,specie2,'.svg', sep=''))
convertSVG(paste('5.Rideogram/',specie1,specie2,'.svg', sep=''), device = "png")
